          ...
          <div class="flex-1">
            ...
            @if (log.changes && log.changes.length > 0) {
              <ul class="mt-1 space-y-1">
                  @for (change of log.changes; track $index) {
                      <li class="text-xs text-zinc-600 dark:text-zinc-300 flex items-center gap-1">
            ...
            <p class="text-xs text-zinc-500 mt-1">{{ log.timestamp | date:'d MMM yyyy, HH:mm' }}</p>
          </div>
        </div>
      } @empty {
